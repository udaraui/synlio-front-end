...
export function TaskSpaceCardSkeleton() {
  return (
    <div className="group relative rounded-xl border-y border-r border-l-2 border-t-gray-200/60 border-b-gray-200/60 border-r-gray-200/60 border-l-gray-300 dark:border-t-gray-700/60 dark:border-b-gray-700/60 dark:border-r-gray-700/60 dark:border-l-gray-600 shadow-sm overflow-hidden">
      <div className="p-4">
        ...
        {/* New Task, Edit, Configure, Delete */}
...
export function TaskSpaceGridSkeleton({ count = 12 }: { count?: number }) {
  ...
  return (
    <div className={`grid gap-3 ${sidebarOpen ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4'}`}>
      {Array.from({ length: count }).map((_, i) => (
        <TaskSpaceCardSkeleton key={i} />
      ))}
    </div>
  );
}

export function TaskSpaceTableSkeleton({ rows = 12 }: { rows?: number }) {
  return (
      ...
      <div className="flex-1 min-h-0 flex flex-col rounded-xl border overflow-hidden bg-white dark:bg-zinc-950">
        <div className="relative flex-1 min-h-0 overflow-hidden">
          ...
          <tr>
            ...
            <th className="h-10 px-4 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">TASKS</th>
        ...
        <tbody>
          {Array.from({ length: rows }).map((_, i) => (
            <tr key={i} className="border-b border-border hover:bg-gray-50 dark:hover:bg-gray-900/50">
