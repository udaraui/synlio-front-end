...
export default function UpcomingTasksPanel({ rows, loading }: Props) {
  const upcomingTasks = useMemo(() => {
    ...
    return rows
      .filter((r) => {
        if (!r.planned_due_date) return false;
        try {
          const d = new Date(r.planned_due_date);
          return d >= now && d <= in7;
        } catch { return false; }
      })
      .sort((a, b) => new Date(a.planned_due_date).getTime() - new Date(b.planned_due_date).getTime());
  }, [rows]);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-border rounded-lg overflow-hidden shadow-sm flex flex-col h-full">
      <div className="px-4 py-3 border-b border-border flex items-center gap-2 flex-shrink-0">
        <h2 className="text-sm font-semibold">Upcoming Tasks (Next 7 Days)</h2>
      </div>
        ...
        <div className="px-4 py-1 flex-1 min-h-[280px]">
          <div className="max-h-56 overflow-y-auto pr-1">
            {upcomingTasks.map((r, i) => (
              <div key={r.task_id ?? i} className="flex items-center gap-2.5 py-2 border-b border-border last:border-0">
                ...
                <p className="flex-1 text-xs font-semibold text-foreground truncate min-w-0">{r.task_name}</p>
